...
Config nltk
'''
os.environ["NLTK_DATA"] = os.path.join(curdir, os.path.pardir, "data", "nltk_data")
...
# run testcase: python /Users/hain/ai/livehelperchat.io/offline/text-classifier/src/common/question_type.py Test.testExample
class Test(unittest.TestCase):
    ...
    def test_nltk_cfg_qtype(self):
        ...
        gfile = os.path.join(curdir, os.path.pardir, "config", "grammar.question-type.cfg")
        ...
        target = []
        for x in sent:
            if x in missing: continue
            target.append(x)

        rd_parser = nltk.RecursiveDescentParser(question_grammar, trace=2)
        result = []
        print("target: ", target)
    ...
    def test_nltk_cfg_en(self):
        ...
        for i, tree in enumerate(rd_parser.parse(sent)):
            result.append(tree)
            save_tree_png(str(tree), os.path.join(curdir, os.path.pardir, "tmp", "nltk_cfg_en_%s.png" % i))
